(() => {
    ...
    (() => {
        ...
        const intl_tel_input_reset = () => {
            input.classList.remove("_form-error");
        };
        input.addEventListener("blur", (() => {
            intl_tel_input_reset();
            if (input.value.trim()) if (iti.isValidNumber()) ; else input.classList.add("_form-error");
        }));
        input.addEventListener("change", intl_tel_input_reset);
        input.addEventListener("keyup", intl_tel_input_reset);
        window["FLS"] = false;
        isWebp();
        formFieldsInit({
            viewPass: false
